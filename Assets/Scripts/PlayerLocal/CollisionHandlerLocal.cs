...
public class CollisionHandlerLocal : MonoBehaviour
{
    ...
    public void ResetToSpawn()
    {
        ...
        else gameObject.transform.position = MultiplayerRunManager.Instance.GetRespawnPoint(gameObject, 2);
    }
    ...
    {
        if (collision.gameObject.GetComponent<SawbladeScript>() != null && !dead)
        {
            if (canGetHurt)
            {
    ...
    private void HurtPlayer(int damage, int hitBy, bool applyKnockback = false, float launchAngle = 0)
    {
        ...
        RpcHurtPlayer(damage, hitBy);
        //apply knockback if needed
        if (applyKnockback)
        {
            TargetLaunch(launchAngle);
    ...
    /// <param name="hitBy">What hit the player? Used for achievements</param>
    /// <param name="applyKnockback">Whether to apply knockback</param>
    void RpcHurtPlayer(int damage, int hitBy)
    {
    ...
    {
        GetComponent<PlayerMovementLocal>().ResetAnimationState();
    }
}
